...
// Function used to wait for keyboard input from user
// Normally the input is get until a newline is inserted.
// If we don't want to wait for newline to get a single char input we have to
// tweak with terminal settings, we use the command cfmakeraw to eliminate the
// caching option with newline in the terminal, tcgetattr is used to get a copy
// of terminal settings before making it raw. with tcsetattr we apply the new
// settings, after we get the char with getchar() we reset the original settings
// from the terminal with tcsetattr once again. Doing so we get a single char
// without waiting for newline and without changing permanently the terminal
// settings
...
const char *vm_state_names[] = {"init",        "idle",      "coin_in",   "stop",
                                "return_coin", "selection", "dispensing"};
...
// valid return states: VM_NO_CHANGE, VM_STATE_IDLE, VM_STATE_COIN_IN,
// VM_STATE_STOP SIGINT triggers an emergency transition to stop
vm_state_t vm_do_idle(vm_state_data_t *data) {
  ...
  if (_exit_request)
    next_state = VM_STATE_STOP;

  return next_state;
}

// Function to be executed in state coin_in
// valid return states: VM_NO_CHANGE, VM_STATE_COIN_IN, VM_STATE_RETURN_COIN,
// VM_STATE_SELECTION SIGINT triggers an emergency transition to stop
vm_state_t vm_do_coin_in(vm_state_data_t *data) {
  ...
  default:
    syslog(LOG_WARNING,
           "[FSM] Cannot pass from coin_in to %s, remaining in this state",
           vm_state_names[next_state]);
    next_state = VM_NO_CHANGE;
  }
...
vm_state_t vm_do_return_coin(vm_state_data_t *data) {
  ...
  default:
    syslog(LOG_WARNING,
           "[FSM] Cannot pass from return_coin to %s, remaining in this state",
           vm_state_names[next_state]);
    next_state = VM_NO_CHANGE;
  }
...
vm_state_t vm_do_selection(vm_state_data_t *data) {
  ...
  printf("Availability: [%d] [%d] [%d] [%d]\n", data->counts[0],
         data->counts[1], data->counts[2], data->counts[3]);
  ...
  default:
    syslog(LOG_WARNING,
           "[FSM] Cannot pass from selection to %s, remaining in this state",
           vm_state_names[next_state]);
    next_state = VM_NO_CHANGE;
  }
...
// valid return states: VM_NO_CHANGE, VM_STATE_IDLE, VM_STATE_RETURN_COIN,
// VM_STATE_DISPENSING SIGINT triggers an emergency transition to stop
vm_state_t vm_do_dispensing(vm_state_data_t *data) {
  ...
  default:
    syslog(LOG_WARNING,
           "[FSM] Cannot pass from dispensing to %s, remaining in this state",
           vm_state_names[next_state]);
    next_state = VM_NO_CHANGE;
  }
