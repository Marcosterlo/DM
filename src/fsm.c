...
Generation date: 2023-05-20 18:52:40 +0200
...
const char *ccnc_state_names[] = {"init",         "idle",         "stop",
                                  "go_to_zero",   "load_block",   "no_motion",
...
transition_func_t
    *const ccnc_transition_table[CCNC_NUM_STATES][CCNC_NUM_STATES] = {
        /* states:           init             , idle             , stop ,
           go_to_zero       , load_block       , no_motion        , rapid_motion
           , interp_motion     */
        /* init          */ {NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL},
        /* idle          */
        {NULL, NULL, NULL, ccnc_begin_zero, ccnc_reset, NULL, NULL, NULL},
        ...
        /* rapid_motion  */
        {NULL, NULL, NULL, NULL, ccnc_end_rapid, NULL, NULL, NULL},
        /* interp_motion */
        {NULL, NULL, NULL, NULL, ccnc_end_interp, NULL, NULL, NULL},
};
...
// valid return states: CCNC_NO_CHANGE, CCNC_STATE_GO_TO_ZERO, CCNC_STATE_IDLE,
// CCNC_STATE_LOAD_BLOCK, CCNC_STATE_STOP SIGINT triggers an emergency
// transition to stop
ccnc_state_t ccnc_do_idle(ccnc_state_data_t *data) {
  ...
  fprintf(stderr, "Press " BGRN "spacebar" CRESET " to run, " BBLU "'z' " CRESET
                  "to set zero, " BRED "'q'" CRESET " to quit\n");
  key = read_key();
  switch (key) {
  case ' ':
...
ccnc_state_t ccnc_do_go_to_zero(ccnc_state_data_t *data) {
  ...
  // 3. CTRL-C may be used for skipping over a rapid motion to 0 (if we are
  // simulating without a real machine or digital twin we will never receive the
  // error value back)
  if (_exit_request) {
    _exit_request = 0;
    next_state = CCNC_STATE_IDLE;
...
void ccnc_begin_zero(ccnc_state_data_t *data) {
  ...
  fprintf(stderr, BYEL "Resetting to zero position...\n" CRESET);
...
void ccnc_end_zero(ccnc_state_data_t *data) {
  ...
  fprintf(stderr, BYEL "Zero position reached\n" CRESET);
